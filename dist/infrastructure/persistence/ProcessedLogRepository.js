...
export class ProcessedLogRepository {
    collectionName = 'processed_logs';
    async save(log) {
        const document = { ...log, _id: undefined };
    ...
    async find(filter = {}, options = {}) {
        ...
        if (filter.activityType)
            q.activityType = filter.activityType;
        if (filter.from || filter.to)
            q.timestamp = {};
        if (filter.from)
            q.timestamp.$gte = filter.from;
        if (filter.to)
            q.timestamp.$lte = filter.to;
